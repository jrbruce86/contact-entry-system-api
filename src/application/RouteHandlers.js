  ...
  return {
    ...
    getAllContacts: (req, res) => {
      contactService.getAllContacts()
        ...
        .catch(error => {
          logError(error);
          res.status(500).send({errorMessage: 'Failed to obtain contacts', error: error})
        })
    },
    ...
    postNewContact: (req, res) => {
      contactService.addNewContact(req.body)
        .then(successfullyInsertedDto => {
          ...
          logError(error);
          res.status(500).send({errorMessage: `Failed to insert contact`, error: error})
      })
    },
    ...
    updateExistingContact: (req, res) => {
      contactService.updateExistingContact(req.body, req.params['id'])
        .then(successfullyUpdatedDto => {
          ...
          logError(error);
          res.status(500).send({errorMessage: 'Failed to update contact', error: error});
      });
    },
    ...
    getCallList: (req, res) => {
      contactService.retrieveCallList()
        .then(successfullyRetrievedCallList => {
          ...
          logError(error);
          res.status(500).send({errorMessage: 'Failed to retrieve call list', error: error})
      });
    },
    ...
    getContact: (req, res) => {
      contactService.getContact(req.params['id'])
        .then(successfullyRetrievedDto => {
          ...
          logError(error);
          res.status(500).send({errorMessage: 'Failed to retrieve contact', error: error})
      });
    },
    ...
    deleteContact: (req, res) => {
      contactService.deleteContact(req.params['id'])
        .then(successfullyDeletedDto => {
          ...
          logError(error);
          res.status(500).send({errorMessage: 'Failed to delete contact', error: error})
      });
    }
...
function logError(error) {
  console.log("Caught error while attempting to update contact.");
  console.log(error);
}
